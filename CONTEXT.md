...
### Response Publisher
The module that delivers a `FormattedResult` to the user via Telegram. If an Active Keyboard ID exists in Session, edits that message's text and reply markup in place via `edit_message_text`. Otherwise (first publish in a turn), posts a new message replying to the Anchor Message and stores its ID as the Active Keyboard ID. Owns the entire Telegram delivery concern including Active Keyboard lifecycle.
